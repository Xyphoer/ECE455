...
__device__ int warp_reduce_sum(int val)
{
    ...
    // After this loop, lane 0 of the warp holds the total sum of that warp.
    return val;
}
...
__global__ void reduce_warp(const int* in, int* out, size_t n_elems) {
    ...
    // Only the first thread in each warp (lane 0) performs the global atomic add.
    // This prevents multiple threads in the same warp from writing duplicates.
