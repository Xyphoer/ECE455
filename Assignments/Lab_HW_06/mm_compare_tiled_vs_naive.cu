...
__global__ void mm_tiled(const T* A, const T* B, T* C, int N)
{
    ...
    // loop through all tiles of A & B needed for this output block
    for (int t = 0; t < (N + TILE_SIZE - 1) / TILE_SIZE; ++t)
    {
        // Load a tile of A & B into shared memory
        ...
        __syncthreads(); // wait for everything to be loaded into memory
